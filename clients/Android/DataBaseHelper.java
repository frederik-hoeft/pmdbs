...
public class DataBaseHelper extends SQLiteOpenHelper {
    ...
    private DataBaseHelper(Context mContext) {
        super(mContext, DB_NAME, null, 48);
    ...
    private static String DB_TBLUSER_COL_BIOPW="U_biometric_password";
    private static String DB_TBLUSER_COL_BIOIV="U_biometric_iv";
    ...
    public void onCreate(SQLiteDatabase db) {
        String query = "PRAGMA foreign_keys = off;BEGIN TRANSACTION;CREATE TABLE Tbl_data (    D_id       INTEGER PRIMARY KEY AUTOINCREMENT,    D_host     TEXT    NOT NULL,    D_url      TEXT,    D_uname    TEXT,    D_password TEXT    NOT NULL,    D_email    TEXT,    D_notes    TEXT,    D_hid      TEXT    DEFAULT EMPTY,    D_datetime TEXT    NOT NULL,    D_icon     TEXT);CREATE TABLE Tbl_delete (    DEL_id  INTEGER PRIMARY KEY AUTOINCREMENT,    DEL_hid TEXT);CREATE TABLE Tbl_settings (    S_id          INTEGER PRIMARY KEY AUTOINCREMENT,    S_server_ip   TEXT,    S_server_port TEXT,    S_auto_sync   INTEGER,    S_keep_alive  INTEGER);CREATE TABLE Tbl_user (    U_id         INTEGER PRIMARY KEY AUTOINCREMENT,    U_username   TEXT,    U_nickname   TEXT    DEFAULT User,    U_password   TEXT,    U_wasOnline  INTEGER DEFAULT (0),    U_firstUsage TEXT    NOT NULL,    U_email      TEXT,    U_cookie     TEXT,    U_privateKey TEXT,    U_publicKey  TEXT,    U_datetime   TEXT    DEFAULT (0), U_biometric_password TEXT DEFAULT 0, U_biometric_iv TEXT DEFAULT 0);COMMIT TRANSACTION;PRAGMA foreign_keys = on;";
    ...
    public void printDB(){
        ...
        try{
            while(c.moveToNext()){
                ...
                System.out.print("D_id: "+ c.getInt(0)+"\t");
                System.out.print("D_host: "+c.getString(1)+"\t");
                System.out.print("D_url: "+c.getString(2)+"\t");
                System.out.print("D_uname: "+c.getString(3)+"\t");
                System.out.print("D_passw: "+c.getString(4)+"\t");
                System.out.print("D_email: "+c.getString(5)+"\t");
                System.out.print("D_notes: "+c.getString(6)+"\t");
                System.out.print("D_hid: "+c.getString(7)+"\t");
                System.out.print("D_datetime: "+c.getString(8)+"\t");
    ...
    public void user_setBiometricInfo(byte[] encrypted_MasterPWHASH, byte[] iv){
        SQLiteDatabase db = this.getWritableDatabase();
        c = null;
        long insertvalue = 0;
        ContentValues cV = new ContentValues();
        ...
        try{
            cV.put(DB_TBLUSER_COL_BIOPW, Base64.encodeToString(encrypted_MasterPWHASH,Base64.NO_WRAP));
            cV.put(DB_TBLUSER_COL_BIOIV,Base64.encodeToString(iv,Base64.NO_WRAP));
            db.update(DB_TBLUSER_NAME,cV,"1=1",null);
        }catch(Exception e){
            msg(e.toString());
        }finally {
            db.close();
    ...
    public List<byte[]> user_getBiometricInfo(){
        SQLiteDatabase db = this.getWritableDatabase();
        c = null;
        c=db.rawQuery("select * from "+DB_TBLUSER_NAME+";",null);
        byte[] encrypted_MasterPWHASH = null;
        byte[] iv = null;

        try{
            while(c.moveToNext()){
                encrypted_MasterPWHASH = Base64.decode(c.getString(11),Base64.NO_WRAP);
                iv = Base64.decode(c.getString(12),Base64.NO_WRAP);
            }
        }catch(Exception e){
            msg(e.toString());
        }finally {
        ...
        List<byte[]> biometricInfo = new ArrayList<>();
        biometricInfo.add(encrypted_MasterPWHASH);
        biometricInfo.add(iv);

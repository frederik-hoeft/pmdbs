...
public class Sync {
    ...
    public static void initialize(String[] params){
        ...
        if(!remoteHeader.equals("headers%eq![]!")){
            ...
            for(int i=0;i<temp_remoteHeaders.size();i++){
                ...
                System.out.println("!!!!!!!!!! SYNC remoteHid: "+remoteHid);
                int remoteTimeStamp = Integer.parseInt(temp_remoteHeaders.get(i)[1]);
                System.out.println("!!!!!!!!!! SYNC remoteTimeStamp: "+remoteTimeStamp);
    ...
    public static void finish(){
        ...
        while(globalVARpool.HID_ThreadCounter>0){
            try {
                Thread.sleep(100);
        ...
        globalVARpool.HID_ThreadCounter=0;
    ...
    public static void setHid(String[] params){
        try{
            ...
            DataBaseHelper db = DataBaseHelper.GetInstance();
            System.out.println("-------------------------BEGIN SET HID----------------");
            db.printDB();

            String query = DataBaseHelper.Security.SQLInjectionCheckQuery(new String[]{"UPDATE Tbl_data SET D_hid = \"",hid,"\" WHERE D_id = ",local_id,";"});

            System.out.println(query);
            //db.modifyData(query);
            System.out.println(db.execSQL(query));
            db.printDB();
            System.out.println("-------------------------END SET HID----------------");

        }catch(Exception e){
            e.printStackTrace();
        }finally {
            if(globalVARpool.HID_ThreadCounter>0) globalVARpool.HID_ThreadCounter--;
